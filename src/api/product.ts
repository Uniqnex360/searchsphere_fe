...
export const fetchProducts = async (filters: {
  ...
  const sortMap: Record<string, { sort_by: string; sort_order: string }> = {
    ...
    "Latest products": { sort_by: "created_at", sort_order: "desc" },
    "Oldest products": { sort_by: "created_at", sort_order: "asc" },
    "Top Reviews": { sort_by: "review", sort_order: "desc" },
...
export const fetchProductsV2 = async (filters: {
  id?: string | number;
  q?: string;
  brand?: string; // Changed from brands to brand to match backend expected key
  category?: string;
  ...
  // 6. API Call
  try {
    const res = await api.get(`product/v7/list/${filters.id}/`, {
      params,
      timeout: 0,
      headers: {
        "X-FE-URL": window.location.href,
